...
import { DiarioTab }     from '@/features/simulation/DiarioTab'
import { PrediccionTab } from '@/features/simulation/PrediccionTab'
import { AyudaTab }      from '@/features/simulation/AyudaTab'
...
export default function SimulationPage({ params }: { params: { id: string; simId: string } }) {
  ...
  const loadSim = useCallback(() =>
    simulationsService.getById(params.simId).then(d => setSim(d as unknown as Sim)),
  [params.simId])
  ...
  useEffect(() => {
    if (!sim) return
    climateService.get(sim.startMonth, sim.startYear)
      .then(({ days, summary }) => {
        if (days) { setClimateDays(days as ClimateDay[]); setClimateSummary(summary) }
      })
      .catch(() => {})
  ...
  async function saveEntry(entry: Parameters<typeof DiarioTab>[0]['onSave'] extends (e: infer E) => unknown ? E : never) {
    if (!sim) return
    await entriesService.save({ simulationId: sim.id, ...entry })
    await loadSim()
